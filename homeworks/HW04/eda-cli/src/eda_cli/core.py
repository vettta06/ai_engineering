...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    has_const_columns = (df.nunique() == 1).any()
    flags['has_constant_columns'] = bool(has_const_columns)

    categorial = df.select_dtypes(include=['object', 'category']).columns
    has_high_category = False
    if len(categorial) > 0:
        maxx = df[categorial].nunique().max()
        has_high_category = maxx > 50
    flags['has_high_cardinality_categoricals'] = bool(has_high_category)
